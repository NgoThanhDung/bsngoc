			...
			<div class="widget-body">
				<?php echo form_open(base_url('admin/news/category/update'), ['class'=>'form-horizontal edit-news-category-form']); ?>
						...
						<div class="col-md-6">
							<select required class="custom-select form-control" name="select_news_category">
								...
								<?php foreach ($categories as $news_category): ?>
									<option value="<?= $news_category['id'] ?>" <?= $category['parent_id'] ==  $news_category['id'] ? "selected" : "" ?>>
										<?= $news_category['name'] ?>
									</option>
								<?php endforeach; ?>
							</select>
					...
					<!-- <div class="form-group row d-flex align-items-center mb-5">
						...
						<div class="col-md-6">
							<input type="text" data-alias-prefix="<?=$alias_prefix ?>" class="form-control alias news_category_alias" name="alias" disabled value="<?=$alias_prefix ?>.<?= $category['alias'] ?>">
						</div>
					</div> -->
					<!-- ========================= -->
					<!-- <div class="form-group row d-flex align-items-center mb-5">
