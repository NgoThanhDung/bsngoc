...
class Menu extends Admin_Controller
{
  ...
  */
  public function render_menu_list_page($type)
  {
    if (!$this->Menu->check_menu_type($type)) {
      show_404();
      return;
    }
    $view_data = [
      ...
      // Lấy danh sách menu trên thanh menu chính
      'menu_list' => $this->Menu->get_menu_list(['type' => $type]),
      'type'      => $type,
      'tab'       => 'menu,menu_'.$type.'_list',
    ];
  ...
  * Render trang danh sách menu
  */
  public function render_create_menu_page($type)
  {
  ...
  */
  ...
  public function create_menu()
  {
    ...
    if (!$this->Menu->create_menu($data_to_insert)) {
      ...
      redirect(base_url('admin/menu/'.$type.'/new'));
    } else {
      ...
      redirect(base_url('admin/menu/'.$type));
    }
    return;
  }
  ...
  public function delete_menu($menu_id)
  {
    ...
    if ($this->Menu->get_children($menu_to_delete['id'])) {
      ...
      redirect(base_url('admin/menu/'.$menu_to_delete['type']));
      return;
    }
  ...
  public function render_edit_menu_page($menu_id) {
    ...
    // lấy danh sách menu cha (khi chỉnh sửa menu item)
    // bỏ qua chính nó
    // bỏ qua con của nó
    $menu_list = $this->Menu->get_menu_list([
      'parent_id !=' => $menu_id,
  ...
  public function update_menu()
  {
    ...
    if (!$this->Menu->update_menu($data_to_update, $menu_id)) {
      ...
      redirect(base_url('admin/menu/'.$menu['type']));
    }
    return;
  }
  ...
  public function ajax_get_links_for_menu()
  {
    $link_type = $this->input->get('link_type');
    ...
    if ($link_type == "product") {
      ...
      $model = "Product";
    }  elseif ($link_type == "product_cate") {
      $this->load->model('Product_category_model', 'ProductCategory');
      $model = "ProductCategory";
    }
    if (!empty($model)) {
      $results = $this->$model->get_links_for_menu($name);
    }
    ...
    if ($results) {
      ...
      $menu_links = $this->load->view('backend/components/menu_links', $view_data, TRUE);
    }


    echo json_encode([
      'menuLinks' => count($results) > 0 ? $menu_links : "Không có kết quả",
    ]);
  }
