...
class Auth extends MY_Controller
{
	...
	public function login_gg()
	{
		...
		$google_client->setClientId('315632901617-d9jr0f438d54nu77s6u4lf95iiphfn5u.apps.googleusercontent.com');
		$google_client->setClientSecret('VRhJ37GPaN8Dhp12tUFaUJGL'); //Define your Client Secret Key
		$google_client->setRedirectUri(base_url('backend/auth/login_gg'));
	...
	public function login($form_data, $view_data)
	{
		...
		$format_pass = md5($pass);



		$user = $this->User->get_user_by_email($email, $format_pass);
		...
		// set_auth_session - applications/core/MY_Controller
		$this->set_auth_session($user);
		redirect(base_url('admin/dashboard'), 'refresh');
	}
	...
	public function register($post, $view_data)
	{
		...
		if ($this->validation->validate_form($post) == FALSE) {
			$this->load->view('backend/auth', $view_data);
			return;
		}
		...
		if (!$insert_result) {
			...
			echo 'Đã xảy ra lỗi không mong muốn. <a href="' + $url + '">Đăng ký lại</a>';
			return;
		}

		$_POST = array();
		$sending_result = $this->send_confirmation_email($post['email_register']);
		// return;
		$msg = "";
		if ($sending_result) {
		...
		$this->session->set_flashdata('email', $post['email_register']);
		$this->session->set_flashdata('msg', $msg);
		redirect(base_url('admin/confirm-email'));
	}
	...
	public function send_confirmation_email($email = '')
	{
		...
		if (empty($email)) {
			echo "Đã xảy ra lỗi không mong muốn. Chúng tôi xin lỗi vì sự bất tiện này";
			return;
		}
