...
class News extends Admin_Controller
{
	...
	public function create_news_category()
	{
		...
		$data_to_create['parent_id'] = $this->input->post('select_news_category');
	...
	public function render_edit_news_category_page($category_id)
	{
		...
		// echo $view_data['alias_prefix'];return;
		$this->load->view('backend/layout', $view_data);
	}
	...
	public function update_news_category()
	{
		...
		$data_to_update['parent_id'] = $this->input->post('select_news_category');
	...
	public function news_datatable_json()
	{
		...
		$news_data = array();
		foreach ($news['data'] as $index => $news_piece) {
			$link = $news_piece['news_alias'];
			$news_data[] = array(
				$index += 1,
				$news_piece['news_name'],
				'<a href="' . base_url($link) . '" target = "_blank">' . $link . '</a>',
				$news_piece['category_name'],
				$news_piece['news_status'] ? '<span class="mr-1 badge badge-success">Hiện</span>' : '<span class="mr-1 badge badge-danger">Ẩn</span>',
